...
const statusConfig = {
  ...
  seen: { color: "text-[#2a9ab8]", bg: "bg-[#e8f7fb]", dot: "bg-[#42AFCA]" },
  reviewing: { color: "text-[#2a9ab8]", bg: "bg-[#e8f7fb]", dot: "bg-[#42AFCA]" },
};
...
export default function Application() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const mapped = apiApps?.length ? apiApps.map((a) => {
          const statusMap = {
            Applied: { status: "application_sent", statusLabel: "Application Sent" },
            Seen: { status: "seen", statusLabel: "Seen" },
            Reviewing: { status: "reviewing", statusLabel: "Reviewing" },
            Shortlisted: { status: "contact_viewed", statusLabel: "Shortlisted" },
          ...
          return {
            ...
            reviewingDate: ['Reviewing', 'Shortlisted', 'Rejected'].includes(a.status) ? new Date(a.updatedAt || a.createdAt).toLocaleDateString() : null,
        ...
        setSelectedApp(mapped.length > 0 ? mapped[0] : null);
      } catch (err) {
  ...
  const steps = selectedApp ? [
    { label: "Application Sent", date: selectedApp.applicationSent, done: true },
    { label: "Seen", date: selectedApp.applicationViewed, done: !!selectedApp.applicationViewed },
    { label: "Reviewing", date: selectedApp.reviewingDate, done: !!selectedApp.reviewingDate },
    { label: "Shortlisted", date: selectedApp.contactViewed, done: !!selectedApp.contactViewed },
  ] : [];
    ...
    <div className="min-h-screen bg-gray-50 font-sans">
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 flex flex-col lg:flex-row gap-4 sm:gap-6">
        ...
        {selectedApp ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-3 sm:p-5 shadow-sm">
              ...
              <div className="mb-6 sm:mb-8">
                ...
                <div className="relative flex items-start">
                  ...
                  />
                  {/* Define steps here, where selectedApp is guaranteed to be non-null */}
                  {(() => {
                    const steps = [
                      { label: "Application Sent", date: selectedApp?.applicationSent, done: true },
                      { label: "Seen", date: selectedApp?.applicationViewed, done: ['Seen', 'Reviewing', 'Shortlisted', 'Rejected'].includes(selectedApp?.rawStatus) },
                      { label: "Reviewing", date: selectedApp?.reviewingDate, done: ['Reviewing', 'Shortlisted', 'Rejected'].includes(selectedApp?.rawStatus) },
                      { label: "Shortlisted", date: selectedApp?.contactViewed, done: ['Shortlisted', 'Rejected'].includes(selectedApp?.rawStatus) },
                    ];
          ...
          <div className={`flex-1 min-w-0 ${mobileView === "list" ? "hidden lg:block" : "block"} flex items-center justify-center`}>
